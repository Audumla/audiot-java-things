...
uint8_t primary_i2c_bus = 1;
uint8_t secondary_i2c_bus = 0;
...
uint32_t bcm2835_peri_read(volatile uint32_t* const  paddr)
{
#ifdef DEBUG
...
uint32_t bcm2835_peri_read_nb(volatile uint32_t* const  paddr)
{
#ifdef DEBUG
...
void bcm2835_peri_write(volatile uint32_t* const  paddr, uint32_t value)
{
#ifdef DEBUG
...
void bcm2835_peri_write_nb(volatile uint32_t* const  paddr, const uint32_t value)
{
#ifdef DEBUG
...
void bcm2835_gpio_fsel(uint8_t pin, uint8_t mode)
{
    ...
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPFSEL0/4;
    uint8_t   shift = (pin % 10) * 3;
    uint32_t  mask = BCM2835_GPIO_FSEL_MASK << shift;
    uint32_t  value = mode << shift;
    bcm2835_peri_set_bits(paddr+pin/10, value, mask);
}

// Set output pin
void bcm2835_gpio_set(uint8_t pin)
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPSET0/4;
    bcm2835_peri_write(paddr+pin/32, 1 << (pin % 32));
}

// Clear output pin
void bcm2835_gpio_clr(uint8_t pin)
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPCLR0/4;
    bcm2835_peri_write(paddr+ pin/32, 1 << (pin % 32));
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPLEV0/4;
    uint8_t shift = pin % 32;
    uint32_t value = bcm2835_peri_read(paddr + pin/32);
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPEDS0/4;
    uint8_t shift = pin % 32;
    uint32_t value = bcm2835_peri_read(paddr + pin/32);
    return (value & (1 << shift)) ? HIGH : LOW;
}
...
void bcm2835_gpio_set_eds(uint8_t pin)
{
    ...
    bcm2835_peri_write(paddr+pin/32, value);
}

// Rising edge detect enable
void bcm2835_gpio_ren(uint8_t pin)
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPREN0/4;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr+pin/32, value, value);
}
void bcm2835_gpio_clr_ren(uint8_t pin)
{
    ...
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr+pin/32, 0, value);
}

// Falling edge detect enable
void bcm2835_gpio_fen(uint8_t pin)
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPFEN0/4;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr + pin/32, value, value);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPHEN0/4;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr + pin/32, value, value);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPLEN0/4;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr + pin/32, value, value);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPAREN0/4 ;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr + pin/32, value, value);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPAFEN0/4 ;
    uint8_t shift = pin % 32;
    uint32_t value = 1 << shift;
    bcm2835_peri_set_bits(paddr + pin/32, value, value);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPPUD/4;
...
{
    volatile uint32_t* const  paddr = bcm2835_gpio + BCM2835_GPPUDCLK0/4;
    uint8_t shift = pin % 32;
    bcm2835_peri_write(paddr + pin/32, (on ? 1 : 0) << shift);
}

// Read GPIO pad behaviour for groups of GPIOs
uint32_t bcm2835_gpio_pad(uint8_t group)
{
    volatile uint32_t* const  paddr = bcm2835_pads + BCM2835_PADS_GPIO_0_27/4;
    return bcm2835_peri_read(paddr + group*2);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_pads + BCM2835_PADS_GPIO_0_27/4;
    bcm2835_peri_write(paddr + group*2, control | BCM2835_PAD_PASSWRD);
}
...
{
    volatile uint32_t* const  paddr = bcm2835_spi0 + BCM2835_SPI0_CLK/4;
...
{
    volatile uint32_t* const  paddr = bcm2835_spi0 + BCM2835_SPI0_CS/4;
    volatile uint32_t* const fifo = bcm2835_spi0 + BCM2835_SPI0_FIFO/4;
...
{
    volatile uint32_t* const  paddr = bcm2835_bsc[bus].paddr + BCM2835_BSC_DIV/4;
...
void bcm2835_i2c_setSlaveAddress(uint8_t addr, uint8_t bus)
{
	...
	volatile uint32_t* const  paddr = bcm2835_bsc[bus].paddr + BCM2835_BSC_A/4;
...
void bcm2835_i2c_set_baudrate(uint32_t baudrate, uint8_t bus)
{
	...
	bcm2835_i2c_setClockDivider(  divider , bus);
}
...
uint8_t bcm2835_i2c_write(char * buf, uint32_t len, uint8_t bus, uint32_t *lenTr)
{
    ...
    // Clear FIFO
    bcm2835_peri_set_bits(bcm2835_bsc[bus].control, BCM2835_BSC_C_CLEAR_1 , BCM2835_BSC_C_CLEAR_1 );
    // Clear Status
	bcm2835_peri_write_nb(bcm2835_bsc[bus].status, BCM2835_BSC_S_CLKT | BCM2835_BSC_S_ERR | BCM2835_BSC_S_DONE);
	// Set Data Length
    bcm2835_peri_write_nb(bcm2835_bsc[bus].dlen, len);
    // pre populate FIFO with max buffer
    while( remaining && ( i < BCM2835_BSC_FIFO_SIZE ) )
    {
        bcm2835_peri_write_nb(bcm2835_bsc[bus].fifo, buf[i]);
        i++;
        remaining--;
    }
    
    // Enable device and start transfer
    bcm2835_peri_write_nb(bcm2835_bsc[bus].control, BCM2835_BSC_C_I2CEN | BCM2835_BSC_C_ST);
    
    // Transfer is over when BCM2835_BSC_S_DONE
    while(!(bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_DONE ))
    {
        while ( remaining && (bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_TXD ))
    	{
        	// Write to FIFO, no barrier
        	bcm2835_peri_write_nb(bcm2835_bsc[bus].fifo, buf[i]);
        	i++;
    ...
    // Received a NACK
    if (bcm2835_peri_read(bcm2835_bsc[bus].status) & BCM2835_BSC_S_ERR)
    {
		reason = BCM2835_I2C_REASON_ERROR_NACK;
    }

    // Received Clock Stretch Timeout
    else if (bcm2835_peri_read(bcm2835_bsc[bus].status) & BCM2835_BSC_S_CLKT)
    {
    ...
    bcm2835_peri_set_bits(bcm2835_bsc[bus].control, BCM2835_BSC_S_DONE , BCM2835_BSC_S_DONE);
    if (lenTr) *lenTr = len - remaining;
    return reason;
}
...
uint8_t bcm2835_i2c_read(char* buf, uint32_t len, uint8_t bus, uint32_t *lenTr)
{
    ...
    bcm2835_peri_write_nb(bcm2835_bsc[bus].dlen, len);
    // Start read
    bcm2835_peri_write_nb(bcm2835_bsc[bus].control, BCM2835_BSC_C_I2CEN | BCM2835_BSC_C_ST | BCM2835_BSC_C_READ);
    
    // wait for transfer to complete
    while (!(bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_DONE))
    {
        // we must empty the FIFO as it is populated and not use any delay
        while (bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_RXD)
    	{
    		// Read from FIFO, no barrier
    		buf[i] = bcm2835_peri_read_nb(bcm2835_bsc[bus].fifo);
        	i++;
        	remaining--;
    	}
    }
    ...
    while (remaining && (bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_RXD))
    {
        // Read from FIFO, no barrier
        buf[i] = bcm2835_peri_read_nb(bcm2835_bsc[bus].fifo);
        i++;
...
{   
	...
	bcm2835_peri_write_nb(bcm2835_bsc[bus].status, BCM2835_BSC_S_CLKT | BCM2835_BSC_S_ERR | BCM2835_BSC_S_DONE);
	// Set Data Length
    bcm2835_peri_write_nb(bcm2835_bsc[bus].dlen, 1);
    // Enable device and start transfer
    bcm2835_peri_write_nb(bcm2835_bsc[bus].control, BCM2835_BSC_C_I2CEN);
    bcm2835_peri_write_nb(bcm2835_bsc[bus].fifo, regaddr[0]);
    bcm2835_peri_write_nb(bcm2835_bsc[bus].control, BCM2835_BSC_C_I2CEN | BCM2835_BSC_C_ST);
    
    // poll for transfer has started
    while ( !( bcm2835_peri_read_nb(bcm2835_bsc[bus].status) & BCM2835_BSC_S_TA ) )
    {
        // Linux may cause us to miss entire transfer stage
        if(bcm2835_peri_read(bcm2835_bsc[bus].status) & BCM2835_BSC_S_DONE)
            break;
    }
...
{   
    ...
    // Set Data Length
    bcm2835_peri_write_nb(bcm2835_bsc[bus].dlen, cmds_len);
 
    // pre populate FIFO with max buffer
    while( remaining && ( i < BCM2835_BSC_FIFO_SIZE ) )
    {
        bcm2835_peri_write_nb(bcm2835_bsc[bus].fifo, cmds[i]);
        i++;
        remaining--;
    }
    ...
    // Send a repeated start with read bit set in address
    bcm2835_peri_write_nb(bcm2835_bsc[bus].dlen, buf_len);
    bcm2835_peri_write_nb(bcm2835_bsc[bus].control, BCM2835_BSC_C_I2CEN | BCM2835_BSC_C_ST  | BCM2835_BSC_C_READ );
    ...
    if (lenTr) *lenTr = buf_len - remaining;
    return reason;
}
...
void *mapmem(char *msg, size_t size, int fd, off_t off)
{
...
int bcm2835_init(void)
{
    ...
    for (bus = 0; bus < 2; ++bus) {
        ...
        bcm2835_bsc[bus].control = bcm2835_bsc[bus].paddr + BCM2835_BSC_C/4;
    }
