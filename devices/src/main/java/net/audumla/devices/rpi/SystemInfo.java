package net.audumla.devices.rpi;
...
public class SystemInfo {
    ...
    private static String getCpuInfo(String target) throws IOException, InterruptedException {
        ...
        if (cpuInfo == null) {
            ...
            String result[] = execute("cat /proc/cpuinfo");
            if (result != null) {
    ...
    public static String getOsFirmwareBuild() throws IOException, InterruptedException {
        String result[] = execute("/opt/vc/bin/vcgencmd version");
        if (result != null) {
            for (String line : result) {
    ...
    public static String getJavaRuntime() {
        return AccessController.doPrivileged((PrivilegedAction<String>) () -> getProperty("java.runtime.name"));
    }
    ...
    public static boolean isHardFloatAbi() {
        ...
        return AccessController.doPrivileged(new PrivilegedAction<Boolean>() {
            ...
            public Boolean run() {
                return contains(getProperty("sun.boot.library.path"), gnueabihf) ||
                        contains(getProperty("java.library.path"), gnueabihf) ||
                        contains(getProperty("java.home"), gnueabihf) ||
    ...
    public static float getCpuTemperature() throws IOException, InterruptedException, NumberFormatException {
        ...
        String result[] = execute("/opt/vc/bin/vcgencmd measure_temp");
        if (result != null) {
            for (String line : result) {
    ...
    private static float getVoltage(String id) throws IOException, InterruptedException, NumberFormatException {
        String[] result = execute("/opt/vc/bin/vcgencmd measure_volts " + id);
        if (result != null) {
            for (String line : result) {
    ...
    private static boolean getCodecEnabled(String codec) throws IOException, InterruptedException {
        String result[] = execute("/opt/vc/bin/vcgencmd codec_enabled " + codec);
        if (result != null) {
            for (String line : result) {
                String parts[] = line.split("=", 2);
    ...
    private static long getClockFrequency(String target) throws IOException, InterruptedException {
        String[] result = execute("/opt/vc/bin/vcgencmd measure_clock " + target.trim());
        if (result != null) {
            for (String line : result) {
                String parts[] = line.split("=", 2);
    ...
    private static String getBashVersionInfo() {
        String versionInfo = "";
        try {
            String result[] = execute("bash --version");
            for (String line : result) {
                if (!line.isEmpty()) {
    ...
    private static String getReadElfTag(String tag) {
        ...
        try {
            String result[] = execute("/usr/bin/readelf -A /proc/self/exe");
            if (result != null) {
                for (String line : result) {
                    line = line.trim();
